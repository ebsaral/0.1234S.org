...
export default function DatePickerSection() {
    ...
    const firstDateTotalSum = Number(dateTotal.toString()[0]) + Number(dateTotal.toString()[1]) + Number(dateTotal.toString()[2]) + Number(dateTotal.toString()[3]);
            ...
            <div className="flex flex-col text-sm gap-4">
                ...
                <div>
                    ...
                    <div>
                        ...
                        {sumInsertion({value: dateTotal})} = {firstDateTotalSum}, {sumInsertion({value: firstDateTotalSum})} ≡ {sum(dateTotal) % mod.value} {mod.text} 
                    </div>
                </div>
                
                <div>
                    <div>{sumInsertion({value: dateArr[0]})} + {sumInsertion({value: dateArr[1]})} + {sumInsertion({value: dateArr[2]})} = {sumOfDateParts}, {sumInsertion({value: sumOfDateParts})} ≡ {`${(sum(dateArr[0] + dateArr[1] + dateArr[2])) % mod.value}`} {mod.text}</div>
                </div>
            </div>
