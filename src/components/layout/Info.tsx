...
export function InfoItem({ title, text, wide }: { title: string; text: string; wide?: boolean }) {
  ...
  return (
    <div className='flex flex-col gap-2 sm:gap-6 sm:flex-row [&:not(:first-child)]:mt-4 sm:[&:not(:first-child)]:mt-2'>
...
export default function Info({ sections }: { sections: ContentSectionItem[] }) {
  return (
    <div className='flex-col space-y-2 gap-6 text-sm text-justify p-5 font-[family-name:var(--font-geist-mono)] whitespace-pre-line max-w-4xl'>
